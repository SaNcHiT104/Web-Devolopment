...
let page
let browserLaunched = puppeteer.launch({ // opening the browser 
    headless:false,
...
browserLaunched.then(function(browserInstance){ 
    ...
    return pageOpen
}).then(function(){ //email written
    let TypeEmailPromise =page.type("input[id='input-1']",email,{delay:100}) //(location,what to type,time duration)
    return TypeEmailPromise 
}).then(function(){ // pass written
    let typePassPromise = page.type("input[id='input-2']",password,{delay:50})
    return typePassPromise
}).then(function(){ // clicking the login button
    ...
    let algorithmPromise = waitAndClick('.topic-card a[data-attr1="algorithms"]',page) //it will wait 
    ...
    let chalangeArr = page.$$('.ui-btn.ui-btn-normal.primary-cta.ui-btn-line-primary.ui-btn-styled',{delay:100}) 
    // array of all the solve challanges button
    return chalangeArr
}).then(function(questionArr){
    console.log('No of question ' + questionArr.length)
    let questionWillBeSolved = questionSolver(page,questionArr[0],codeFile.answers[0])
})
...
function waitAndClick(selector,cpage){
    return new Promise(function(resolve,reject){ //current page ke liye wait krega, algorithm milega click krega
...
function questionSolver(page,question,answer){
    return new Promise(function(resolve,reject){
        ...
        questionWillbeClicked.then(function(){
            let waitForEditor = waitAndClick('.monaco-editor.no-user-select.vs',page) // just to ensure we are on right page
        ...
        }).then(function(){ //getting on the editor box
